...
1. `ruff format --check` — no formatting diff.
2. `ruff check` — lint clean.
3. `ty` (or `mypy`) — type-clean.
4. `pytest` — tests pass.

**Required before merging:**

5. Tests cover the new behavior, not just exercise it.
6. Failure modes are tested — not only the happy path.

**Don't:**

- Mock what you don't own. Use real fixtures + real I/O where you
  can. Mocks of external libraries tend to lie about contracts.
- Pin to internal call shape — assert on the public return /
  side-effect.
...
- **Testing the language/framework itself.** Python's stdlib, pytest, ruff are mature.
...
`.pre-commit-config.yaml` runs the core checks before each commit.
Install with `uvx pre-commit install`.

`.github/workflows/quality.yml` runs the same checks on push and PR.
...
- [ ] `ruff check` clean
- [ ] Type checker (`ty` or `mypy`) clean
- [ ] `pytest` passes locally
- [ ] New behavior covered by tests, not just exercised
- [ ] Failure modes tested (not only the happy path)
- [ ] `STATE.md` updated if the change is operational
