#!/usr/bin/env bash
# Launch the morphogen agent container with the right bind-mounts.
#
...
IMAGE="${AGENT_CONTAINER_IMAGE:-morphogen-agent:latest}"
...
# /workspace/morphogen, so the in-container project dir is
# named `-workspace-morphogen`. Bind-mount the host's
...
CONTAINER_PROJECT_NAME="-workspace-morphogen"
...
# already-built venv).
VENV_VOLUMES=(
    -v morphogen-agent-venv-queue:/workspace/morphogen/dev-tools/queue/.venv
)
...
if [[ -f "$REPO_ROOT/.claude/settings.local.json" ]]; then
    HOST_SETTINGS_MASK+=(-v /dev/null:/workspace/morphogen/.claude/settings.local.json:ro)
fi
...
exec docker run --rm "${TTY_FLAGS[@]}" \
    --hostname morphogen-agent \
    -v "$REPO_ROOT:/workspace/morphogen" \
    "${HOST_SETTINGS_MASK[@]}" \
    "${VENV_VOLUMES[@]}" \
    -v morphogen-agent-home:/home/agent \
    -v "$HOST_PROJECT_DIR:/home/agent/.claude/projects/$CONTAINER_PROJECT_NAME" \
    -w /workspace/morphogen \
    "$IMAGE" \
    "$@"
