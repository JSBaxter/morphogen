...
|------------------------------|---------------------------------------------------|------|-----|
| repo root                    | `/workspace/morphogen`                   | rw   | The cell repo. |
| `~/.claude/projects/<repo>`  | `/home/agent/.claude/projects/-workspace-morphogen` | rw   | Sessions + memory persist across container runs. |
...
|-----------------------------------------|---------------------------------------------------------|---------------|
| `morphogen-agent-home`         | `/home/agent`                                           | The whole agent home dir. Includes `~/.claude/` (OAuth tokens, session metadata) AND `~/.claude.json` (Claude Code's user-config file at the home root — the first-run wizard re-prompts forever if this isn't persisted). |
| `morphogen-agent-venv-queue`   | `/workspace/morphogen/dev-tools/queue/.venv`   | uv project venv for the bundled queue |
...
```bash
docker volume rm morphogen-agent-home
...
docker build -t morphogen-agent:latest dev-tools/agent-container/
...
- The repo is at `/workspace/morphogen`. `git status` works
  as expected.
...
```bash
AGENT_CONTAINER_IMAGE=morphogen-agent:latest \
    dev-tools/agent-container/run.sh
```
